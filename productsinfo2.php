...
if (isset($_POST['add'])):
    ...
    setcookie("ybaCart[" . ($cookie_id + 1) . "][products_id]", $data['products_id'], time() + (86400 * 30), "/");
    setcookie("ybaCart[" . ($cookie_id + 1) . "][customers_basket_quantity]", $data['customers_basket_quantity'], time() + (86400 * 30), "/");

    if(empty($data['title'])){
        setcookie("ybaCart[" . ($cookie_id + 1) . "][title]", " ", time() + (86400 * 30), "/");
    }else{
        setcookie("ybaCart[" . ($cookie_id + 1) . "][title]", $data['title'], time() + (86400 * 30), "/");
    }

    if(empty($data['background'])){
        setcookie("ybaCart[" . ($cookie_id + 1) ."][background]", " ", time() + (86400 * 30), "/");
    }else{
        setcookie("ybaCart[" . ($cookie_id + 1) ."][background]", $data['background'], time() + (86400 * 30), "/");
    }

    if(empty($data['footer'])){
        setcookie("ybaCart[" . ($cookie_id + 1) ."][footer]", " ", time() + (86400 * 30), "/");
    }else{
        setcookie("ybaCart[" . ($cookie_id + 1) ."][footer]", $data['footer'], time() + (86400 * 30), "/");
    }

    if(empty($data['upload'])){
        setcookie("ybaCart[" . ($cookie_id + 1) ."][upload]", " ", time() + (86400 * 30), "/");
    }else{
        setcookie("ybaCart[" . ($cookie_id + 1) ."][upload]", $data['upload'], time() + (86400 * 30), "/");
    }

    if(empty($data['customs'])){
        setcookie("ybaCart[" . ($cookie_id + 1) ."][customs]", " ", time() + (86400 * 30), "/");
    }else{
        setcookie("ybaCart[" . ($cookie_id + 1) ."][customs]", $data['customs'], time() + (86400 * 30), "/");
    }
