...
<head>
    ...
    <meta name="youthbowlingawards" content="youthbowlingawards accessories">
...
INNER JOIN zen_categories d
ON d.categories_id = c.categories_id', 'd.parent_id = 520');
            foreach ($products as $row) {
                //boostpromotions IP
                echo "<li class='tag hvr-grow-shadow'><a class='thumbnail' href='productsinfo.php?products_id={$row["products_id"]}'>
                    <img src='http://50.87.226.168/images/{$row["products_image"]}' alt='youthbowlingawards-{$row["products_name"]}' />
                    <h6>{$row["products_model"]}</h6>
                    <p>{$row["products_name"]}</p></a>
                                                        </li>";
            }
            ?>
